...
public class BasicDistributedZkTest extends AbstractFullDistribZkTestBase {
  ...
  private void testShardParamVariations() throws Exception {
    ...
    for (String shard : shardToJetty.keySet()) {
      ...
      for (SolrServer client : solrclients) {
        ...
        long numDocs = client.query(query, METHOD.GET, SEARCH_CREDENTIALS).getResults().getNumFound();
        ...
        for (CloudJettyRunner replicaJetty : shardToJetty.get(shard)) {
          ...
          numDocs = client.query(query, METHOD.GET, SEARCH_CREDENTIALS).getResults().getNumFound();
    ...
    for (SolrServer client : this.clients) {
      assertEquals("numDocs for "+randShards+" via "+client,
                   ...
                   client.query(query, METHOD.GET, SEARCH_CREDENTIALS).getResults().getNumFound());
    }
  ...
  private void testUpdateProcessorsRunOnlyOnce(final String chain) throws Exception {
    ...
    for (int i = 1; i < numLoops; i++) {
      ...
      ures = updateClient.commit(true, true, false, UPDATE_CREDENTIALS);
  ...
  private void testANewCollectionInOneInstanceWithManualShardAssignement() throws Exception {
   ...
   // solrj.getZkStateReader().getLeaderUrl(oneInstanceCollection2, "slice2", 30000);
    ...
    long oneDocs = client1.query(query, METHOD.GET, SEARCH_CREDENTIALS).getResults().getNumFound();
    long twoDocs = client2.query(query, METHOD.GET, SEARCH_CREDENTIALS).getResults().getNumFound();
    long threeDocs = client3.query(query, METHOD.GET, SEARCH_CREDENTIALS).getResults().getNumFound();
    long fourDocs = client4.query(query, METHOD.GET, SEARCH_CREDENTIALS).getResults().getNumFound();
    
    query.set("collection", oneInstanceCollection2);
    query.set("distrib", true);
    long allDocs = getCommonCloudSolrServer().query(query, METHOD.GET, SEARCH_CREDENTIALS).getResults().getNumFound();
...
//    assertNotSame(oneDocs, twoDocs);
  ...
  private void testSearchByCollectionName() throws SolrServerException {
    ...
    SolrServer client = clients.get(0);
    final String baseUrl = getBaseUrl(client);
...
//    System.out.println("All Docs:" + allDocs);
  ...
  private void testMultipleCollections() throws Exception {
    ...
    getCommonCloudSolrServer().add(getDoc(id, "10000004"), -1, UPDATE_CREDENTIALS);
    ...
    getCommonCloudSolrServer().add(getDoc(id, "10000005"), -1, UPDATE_CREDENTIALS);
    getCommonCloudSolrServer().setDefaultCollection(null);
    
    otherCollectionClients.get("collection2").get(0).commit(true, true, false, UPDATE_CREDENTIALS);
    otherCollectionClients.get("collection3").get(0).commit(true, true, false, UPDATE_CREDENTIALS);
    
    getCommonCloudSolrServer().setDefaultCollection("collection1");
    long collection1Docs = getCommonCloudSolrServer().query(new SolrQuery("*:*"), METHOD.GET, SEARCH_CREDENTIALS).getResults()
        .getNumFound();
    long collection2Docs = otherCollectionClients.get("collection2").get(0)
        .query(new SolrQuery("*:*"), METHOD.GET, SEARCH_CREDENTIALS).getResults().getNumFound();
    ...
    long found = clients.get(0).query(query, METHOD.GET, SEARCH_CREDENTIALS).getResults().getNumFound();
    assertEquals(collection2Docs + collection3Docs, found);
    
    query = new SolrQuery("*:*");
    query.set("collection", "collection1,collection2,collection3");
    found = clients.get(0).query(query, METHOD.GET, SEARCH_CREDENTIALS).getResults().getNumFound();
    assertEquals(collection1Docs + collection2Docs + collection3Docs, found);
    
    // try to search multiple with cloud client
    found = getCommonCloudSolrServer().query(query, METHOD.GET, SEARCH_CREDENTIALS).getResults().getNumFound();
    assertEquals(collection1Docs + collection2Docs + collection3Docs, found);
  ...
  protected QueryResponse queryServer(ModifiableSolrParams params) throws SolrServerException {
    ...
    QueryResponse rsp = getCommonCloudSolrServer().query(params, METHOD.GET, SEARCH_CREDENTIALS);
    return rsp;
  }
