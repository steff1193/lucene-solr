...
public class FullSolrCloudDistribCmdsTest extends AbstractFullDistribZkTestBase {
  ...
  public void doTest() throws Exception {
    ...
    controlClient.deleteById(Long.toString(docId-1), -1, UPDATE_CREDENTIALS);
    controlClient.deleteById(Long.toString(docId-2), -1, UPDATE_CREDENTIALS);
  ...
  private void testOptimisticUpdate(QueryResponse results) throws Exception {
    ...
    QueryResponse res = clients.get(0).query(params, METHOD.GET, SEARCH_CREDENTIALS);
    assertEquals(0, res.getResults().getNumFound());
    
    params = new ModifiableSolrParams();
    params.add("q", t1 + ":theupdatestuff");
    res = clients.get(0).query(params, METHOD.GET, SEARCH_CREDENTIALS);
    assertEquals(1, res.getResults().getNumFound());
  }

  private QueryResponse query(SolrServer server) throws SolrServerException {
    SolrQuery query = new SolrQuery("*:*");
    return server.query(query, METHOD.GET, SEARCH_CREDENTIALS);
  }
